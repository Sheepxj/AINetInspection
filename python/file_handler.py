        ...
        if file:
            ...
            with zipfile.ZipFile(file_path, 'r') as zip_ref:
                for member in zip_ref.namelist():
                    ...
                    # 构建目标路径
                    target_path = os.path.join(zip_extract_folder, member)
                    # 创建目标路径的目录（如果需要）
                    os.makedirs(os.path.dirname(target_path), exist_ok=True)
                    # 提取文件
                    zip_ref.extract(member, zip_extract_folder)
...
def tosend_file(request):
    ...
    model_path = os.path.join(MODEL_FOLDER, path, 'weights', 'Protection.pt')
